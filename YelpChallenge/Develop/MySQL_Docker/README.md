...
To create the image , execute the following command on the tutum-mysql folder:
...
on the container to store the database files. To do so:
	...
	docker run -d -v /path/in/host:/var/lib/mysql /bin/bash -c "/usr/bin/mysql_install_db"
...
After this you can start your mysql image but this time using `/path/in/host` as the database folder:
	...
	docker run -d -p 3306:3306 -v /path/in/host:/var/lib/mysql
